package memory

import (
	"bytes"
	"context"
	"errors"
	"testing"
)

func TestFindAll(t *testing.T) {
	pattern := "8B 45 ?? C7 00 00 00 ?? ?? 5D C2 08 00 8B 4D"
	data := []byte{0x69, 0x69, 0x69, 0x8B, 0x45, 0x0C, 0xC7, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x5D, 0xC2, 0x08, 0x00, 0x8B, 0x4D}
	want := 3

	i, err := FindAll(pattern, data)
	if len(i) != 1 && i[0] != want || err != nil {
		t.Errorf("Find(%q, %s): want %d, got %d", pattern, data, want, i)
	}

	pattern = "8B 45 ?? C7 00 00 00 ?? ?? 5D C2 08 00 8B 4D"
	data = []byte{0x69, 0x69, 0x69, 0x69, 0x8B, 0x45, 0x0C, 0xC7, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x5D, 0xC2, 0x08, 0x00, 0x8B, 0x4D}
	want = 4

	i, err = FindAll(pattern, data)
	if len(i) != 1 && i[0] != want || err != nil {
		t.Errorf("Find(%q, %s): want %d, got %d", pattern, data, want, i)
	}

	pattern = "8B 45 ?? C7 00 00 00 ?? ?? 5D C2 08 00 8B 4D"
	data = []byte{0x8B, 0x45, 0x69, 0xC7, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x5D, 0xC2, 0x08, 0x00, 0x8B, 0x4D}
	want = 0

	i, err = FindAll(pattern, data)
	if len(i) != 1 && i[0] != want || err != nil {
		t.Errorf("Find(%q, %s): want %d, got %d", pattern, data, want, i)
	}
}

func TestFindAll_Reader(t *testing.T) {
	pattern := "8B 45 ?? C7 00 00 00 ?? ?? 5D C2 08 00 8B 4D"
	fakeProcess := &fakeReader{}
	fakeProcess.data = append(fakeProcess.data, bytes.Repeat([]byte{0x69}, 69)...)
	fakeProcess.data = append(fakeProcess.data, 0x8B, 0x45, 0x69, 0xC7, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x5D, 0xC2, 0x08, 0x00, 0x8B, 0x4D)
	fakeProcess.data = append(fakeProcess.data, bytes.Repeat([]byte{0x69}, 1024)...)
	reader := NewBufferedReader(fakeProcess, Pointer{Addrs: []uintptr{0}})

	// TODO: need to fix, it is getting the wrong match addr
	matches, err := FindAllReader(pattern, reader)
	if err != nil {
		t.Fatalf("want no error, got %v", err)
	}

	if len(matches) != 1 {
		t.Fatalf("len(matches) = %d, want 1", len(matches))
	}

	if matches[0].Addrs[0] != 69 {
		t.Fatalf("matches[0].Addrs[0] = %d, want 69", matches[0].Addrs[0])
	}
}

type fakeReader struct {
	data []byte
}

func (o *fakeReader) ReadFromAddr(_ context.Context, addr Pointer, size uint64) ([]byte, error) {
	if len(addr.Addrs) == 0 {
		return nil, errors.New("invalid address")
	}

	offset := uint64(addr.Addrs[len(addr.Addrs)-1])
	if offset > uint64(len(o.data)) {
		return nil, errors.New("invalid offset")
	}

	upto := offset + size
	if upto > uint64(len(o.data)) {
		return nil, errors.New("size out of range")
	}

	return o.data[offset:upto], nil
}
